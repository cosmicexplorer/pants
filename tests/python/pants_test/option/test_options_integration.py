...
class TestOptionsIntegration(PantsRunIntegrationTest):
  ...
  def test_no_baseurls_options_only(self):
    ...
    self.assert_success(pants_run)
    empty_baseurls_output = 'binaries_baseurls = [] (from ENVIRONMENT from env var PANTS_BINARIES_BASEURLS)'
    self.assertIn(empty_baseurls_output, pants_run.stdout_data)

  def test_set_baseurls_cmdline(self):
    pants_run = self.run_pants([
      '--binaries-baseurls="[]"',
      'options',
    ])
