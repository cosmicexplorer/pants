...
from pants.backend.native.rules.cpp_rules import CppSources, CppObjectsRequest, CppObjects, create_cpp_rules
...
class TestNativeToolchain(BaseTest, SchedulerTestBase):
  ...
  def setUp(self):
    ...
    clang_inst = global_subsystem_instance(Clang)

    rules = create_fs_rules() + create_cpp_rules(
      clang_inst=clang_inst,
    )
...
#     self.assertEqual(c_output, 'hello, world!\n')
  ...
  def test_hello_cpp(self):
    cpp_src = self.create_file('hello.cpp', contents="""
#include <iostream>
...
""")
    ...
    cpp_dir = os.path.dirname(cpp_src)

    cpp_request = CppObjectsRequest(
      sources=CppSources(
        root_dir=cpp_dir,
        file_paths=['hello.cpp'],
      ),
      dir_path=cpp_dir,
    )

    hello_objs = self.execute(self.scheduler, CppObjects, cpp_request)
    # hello_objs = self.scheduler.product_request(CppObjects, [cpp_request])

    raise Exception("objs: {}".format(repr(hello_objs)))
