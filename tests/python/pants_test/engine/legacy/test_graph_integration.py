...
class GraphIntegrationTest(PantsRunIntegrationTest):
  ...
  def test_exception_with_global_option(self):
    ...
    self.assertIn(
      """SourcesGlobMatchError: In target testprojects/src/python/sources:some-missing-some-not with sources=globs('*.txt', '*.rs'): Some globs failed to match and --glob-match-failure is set to GlobMatchErrorBehavior(error_on_glob_match_failure<=str>=error). The failures were:
    ...
    self.assert_failure(pants_run)
    self.assertIn(
      """SourcesGlobMatchError: In target testprojects/src/java/org/pantsbuild/testproject/bundle:missing-bundle-fileset with fileset=rglobs('*.aaaa', '*.bbbb'): Some globs failed to match and --glob-match-failure is set to GlobMatchErrorBehavior(error_on_glob_match_failure<=str>=error). The failures were:
