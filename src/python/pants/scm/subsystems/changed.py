...
async def find_owners(
    ...
    owners = await Get[Owners, OwnersRequest](OwnersRequest(sources=changed_request.sources))
    ...
    all_addresses = await Get[Addresses, AddressSpecs](AddressSpecs((DescendantAddresses(""),)))
    all_structs = [
