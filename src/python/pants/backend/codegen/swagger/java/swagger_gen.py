...
class SwaggerGen(SimpleCodegenTask):

  sources_globs = ('**/*',)

  @classmethod
  def subsystem_dependencies(cls):
    return super().subsystem_dependencies() + (Swagger.scoped(cls),)

  @classmethod
  def register_options(cls, register):
    super().register_options(register)

    # The swagger plugin names are used as proxies for the identity of the swagger
    # executable environment here.  Plugin authors must include a version in the name for
    # proper invalidation of swagger products in the face of plugin modification that affects
    # plugin outputs.
    register('--swagger-plugins', advanced=True, fingerprint=True, type=list,
             help='Names of swagger plugins to invoke.  Swagger will look for an executable '
                  'named swagger-gen-$NAME on PATH.')
    register('--extra_path', advanced=True, type=list,
             help='Prepend this path onto PATH in the environment before executing swagger. '
                  'Intended to help swagger find its plugins.',
             default=None)
    register('--javadeps', advanced=True, type=list,
             ...
             default=['3rdparty:swagger-java'])
    register('--import-from-root', type=bool, advanced=True,
             help='If set, add the buildroot to the path swagger searches for imports. '
                  'This enables using import paths relative to the build root in .swagger files, '
                  'as recommended by the swagger documentation.')
  ...
  def __init__(self, *args, **kwargs):
    """Generates Java files from swagger files using the swagger compiler."""
    super().__init__(*args, **kwargs)
    self.plugins = self.get_options().swagger_plugins or []
    self._extra_paths = self.get_options().extra_path or []

  @property
  def swagger_binary(self):
  ...
  def is_gentarget(self, target):
    return isinstance(target, JavaSwaggerLibrary)
  ...
  def execute_codegen(self, target, target_workdir):
    ...
    bases = OrderedSet()
    # Note that the root import must come first, otherwise swagger can get confused
    ...
    bases.update(self._swagger_path_imports([target]))

    gen_flag = '--java_out'

    gen = '{0}={1}'.format(gen_flag, target_workdir)

    args = [self.swagger_binary, gen]
    ...
    for base in bases:
      args.append('--swagger_path={0}'.format(base))

    args.extend(sources)

    # Tack on extra path entries. These can be used to find swagger plugins.
    swagger_environ = os.environ.copy()
    if self._extra_paths:
      swagger_environ['PATH'] = os.pathsep.join(self._extra_paths
                                               + swagger_environ['PATH'].split(os.pathsep))
    ...
    with self.context.new_workunit(name='swagger',
      ...
      if result != 0:
        raise TaskError('{} ... exited non-zero ({})'.format(self.swagger_binary, result))
  ...
  def _swagger_path_imports(self, swagger_targets):
    for target in swagger_targets:
