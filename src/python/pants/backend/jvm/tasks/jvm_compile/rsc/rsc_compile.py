...
class RscCompile(ZincCompile):
  ...
  class _JvmTargetType(enum(['zinc', 'rsc'])): pass
    ...
    def work_for_vts_rsc(vts, ctx):
      ...
      if not hit_cache:
        ...
        with Timer() as timer:
          ...
          (input_digest, classpath_entry_paths, distribution) = self.do_for_execution_strategy_variant({
  ...
  def _runtool_hermetic(self, main, tool_name, args, distribution, with_nailgun=False, tgt=None, input_files=tuple(), input_digest=None, output_dir=None):
    ...
    # NB: Need to run the merged rsc-and-zinc.jar in branch `remote-nailgun-rsc-and-zinc` at SHA at
    # least fa8584e with the following command:
    # ./pants --resolver-resolver=coursier --coursier-repos="['https://artifactory-ci.twitter.biz/libs-releases-local/','https://artifactory-ci.twitter.biz/repo1.maven.org/','https://artifactory-ci.twitter.biz/java-virtual/']" binary src/scala/org/pantsbuild/zinc/compiler:rsc-and-zinc
    if with_nailgun:
      cmd = [
        './ng', main,
    ...
    epr_input_files = self.context._scheduler.merge_directories(
      ...
      + ((Digest("c2547bbae7598ac25ee97db829317c7356ff696a478f2daf37cfd449f0a9eaeb", 80),)
         if with_nailgun else ()))
