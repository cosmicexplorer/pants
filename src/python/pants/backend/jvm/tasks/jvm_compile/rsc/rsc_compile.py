...
class RscCompile(ZincCompile, MirroredTargetOptionMixin):
  ...
  def create_compile_context(self, target, target_workdir):
    ...
    rel_workdir = fast_relpath(target_workdir, get_buildroot())
    rsc_dir = os.path.join(rel_workdir, "rsc")
    zinc_dir = os.path.join(rel_workdir, "zinc")
