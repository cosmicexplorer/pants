...
from pants.backend.python.tasks.prepare_local_python_distribution_artifacts import \
  PrepareLocalPythonDistributionArtifacts
...
def register_goals():
  ...
  task(name='py-wheels', action=PrepareLocalPythonDistributionArtifacts).install('binary')
