...
class GlobMatchErrorBehavior(datatype([('error_on_glob_match_failure', str)])):
  ...
  def __new__(cls, *args, **kwargs):
    ...
    if this_object.error_on_glob_match_failure not in cls.allowed_values:
      raise cls.make_type_error(
        "Value {!r} for error_on_glob_match_failure must be one of: {!r}."
        .format(this_object.error_on_glob_match_failure, cls.allowed_values))

    return this_object

  def should_log_warn_on_error(self):
    if self.error_on_glob_match_failure in ['warn', 'error']:
      return True
    else:
      return False
  ...
  def should_throw_on_error(self, warnings):
    if not warnings:
      return False
    elif self.error_on_glob_match_failure == 'error':
      return True
    else:
      return False
