...
class ChangesSince(QueryParser):
    ...
    def parse_from_args(
        ...
        return cls(
            changes_since=str(changes_since),
...
async def changes_since_request(
    scm_wrapper: UncachedScmWrapper, changes_since: ChangesSince,
) -> HydratedOperator:
    scm = scm_wrapper.scm
    ...
    changed = await Get[ChangedAddresses](
        ChangedRequest(
...
class FilterOperands(QueryOperation):
    ...
    def apply_filter(self) -> Addresses:
        return Addresses(tuple(t.adaptor.address for t in self.hts if self.filter_func(t.adaptor)))
...
class TypeFilter(QueryParser):
    ...
    def quick_operator(self):
        return FilterOperator(lambda t: t.type_alias in self.allowed_type_aliases)
...
class AddressRegexFilter(QueryParser):
    ...
    def quick_operator(self):
        return FilterOperator(
            lambda t: not any(re.search(rx, t.adaptor.address.spec) for rx in self.regexes)
        )
...
async def minimize_results(operands: MinimizeOperands) -> IntermediateResults:
    ...
    dep_roots = OrderedSet(dep.address for ht in hts for dep in ht.adaptor.dependencies)
